...
<script setup lang="ts">
    ...
    watch(()=>props.modelValue, ()=>{
        props.editId ? val.value = store.getters.getCategoriesById(props.editId) : val.value = ''
    })
    ...
    const ok = () => {
        ...
        if (props.editId) {
            loading.value = true
            store.dispatch(MainActions.EDIT_CATEGORIES, {id: props.editId, name: val.value})
                .finally(()=>{
                        loading.value = false
            ...
            loading.value = true
            store.dispatch(MainActions.ADD_CATEGORIES, {name: val.value})
                .finally(()=>{
                        loading.value = false
                        store.dispatch(MainActions.GET_CATEGORIES)
                        emits('update:modelValue', false)
